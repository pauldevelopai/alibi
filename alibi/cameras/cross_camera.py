...
# How close an appearance has to be before it is called the SAME thing.
#
# One number for everything was wrong. Measured on this site's own galleries,
# DIFFERENT vehicle clusters score 0.74–0.81 against each other — well over the
# old 0.6 bar — which is how one "car" ended up holding thousands of sightings
# of several different vehicles in different places. Overhead vehicle crops
# simply do not separate the way faces do.
#
# So vehicles get a much stricter bar. That deliberately trades one failure for
# the other: a strict bar splits ONE car into several clusters, which the owner
# can rejoin by giving them the same name. A loose bar silently merges TWO cars
# under one name, which nobody can see or undo. Fragmentation is recoverable;
...
# Keep several views per entity rather than one averaged vector. A running mean
# drifts: absorb one wrong car and the centre moves toward it, which makes the
# next wrong car easier to absorb. Views are compared best-of, the way faces
# already are.
MAX_GALLERY_VIEWS = 8
# A new view only earns its place if it shows the thing from a genuinely
# different angle — otherwise the gallery fills with near-duplicates.
VIEW_NOVELTY = 0.97


def _as_views(ref: Any) -> np.ndarray:
    """A gallery entry as (N, D), whatever shape it was stored in.

    Entries written before multi-view galleries are a single 1-D vector; they
    read back as one view, so an existing install keeps working untouched.
    """
...
class CrossCameraTracker:
            ...
            try:
                for etype, entities in (snapshot.get("galleries") or {}).items():
                    for eid, vec in entities.items():
                        # Always (N, D): a flat list from an older install
                        # becomes a single view.
                        self._galleries[etype][eid] = _as_views(vec)
    ...
    def save_galleries(self) -> None:
        ...
        try:
            snapshot = {
                ...
                # Nested lists now — one row per view. A single flat list from
                # an older install still loads (see _as_views).
                "galleries": {etype: {eid: _as_views(vec).tolist()
                                      for eid, vec in entities.items()}
    ...
    def record_appearance_sighting(
        ...
        if match_threshold is None:
            match_threshold = MATCH_THRESHOLDS.get(entity_type, DEFAULT_MATCH_THRESHOLD)
        emb = None
        ...
        if entity_id is None:
            ...
            if emb is not None:
                self._galleries[entity_type][entity_id] = l2_normalize(emb).reshape(1, -1)
    ...
    def _resolve_entity_by_appearance(
        ...
        # Each entity holds SEVERAL views; an entity scores as its best-matching
        # view. One averaged vector blurred a car's front and rear into a shape
        # that resembled other cars more than either real view did.
        best_id, best_score = None, 0.0
        for eid, ref in gallery.items():
            views = _as_views(ref)
            # Only entities embedded by the SAME model are comparable. Swap the
            # ReID backend and the old vectors become meaningless here.
            if views.ndim != 2 or views.size == 0 or views.shape[1] != q.size:
                continue
            norms = np.linalg.norm(views, axis=1)
            norms[norms == 0] = 1.0
            s = float(np.max((views @ q) / norms))
            if s > best_score:
    ...
    def _update_gallery(self, entity_type: str, entity_id: str, emb: np.ndarray) -> None:
        """Add this look to the entity's gallery, keeping several distinct views.

        The old running mean drifted: absorbing one wrong match pulled the
        centre toward it, which made the next wrong match easier to absorb —
        how a single "car" came to hold thousands of sightings of several
        different vehicles. Views are kept separately and matched best-of, so
        one bad frame can never contaminate the rest.
        """
        v = l2_normalize(np.asarray(emb, dtype=np.float32).ravel())
        ref = self._galleries[entity_type].get(entity_id)
        if ref is None:
            self._galleries[entity_type][entity_id] = v.reshape(1, -1)
            self._gallery_counts[entity_type][entity_id] = 1
            return

        views = _as_views(ref)
        if views.shape[1] != v.size:            # different embedder — start over
            self._galleries[entity_type][entity_id] = v.reshape(1, -1)
            self._gallery_counts[entity_type][entity_id] = 1
            return

        # Only keep a genuinely new angle; near-duplicates add nothing but cost.
        norms = np.linalg.norm(views, axis=1)
        norms[norms == 0] = 1.0
        if float(np.max((views @ v) / norms)) >= VIEW_NOVELTY:
            return
        views = np.vstack([views, v.reshape(1, -1)])[-MAX_GALLERY_VIEWS:]
        self._galleries[entity_type][entity_id] = views
        self._gallery_counts[entity_type][entity_id] = int(views.shape[0])
