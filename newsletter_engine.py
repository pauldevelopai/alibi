...
# ============================================================================
# Citation Validation (Deterministic - No Model Calls)
# ============================================================================

def validate_citations(draft_text: str, evidence_ids: List[str]) -> Dict[str, Any]:
    """
    Deterministic validation of citations in generated text.
    
    Checks:
    1. Invalid citation IDs (citations not in evidence_ids)
    2. Missing citations (factual sentences without citations)
    
    Args:
        draft_text: Generated newsletter text
        evidence_ids: List of valid citation IDs like ['PS_1', 'VP_2', 'KB_3']
    
    Returns:
        Dict with keys:
        - missing_citations_sentences: List[str] - factual sentences without citations
        - invalid_citation_ids: List[str] - citation IDs not in evidence_ids
        - citation_count: int - total valid citations found
        - passed: bool - True if no issues found
    """
    # Extract all citations with regex: [ID]
    citation_pattern = r'\[([^\]]+)\]'
    found_citations = re.findall(citation_pattern, draft_text)
    
    # Find invalid citation IDs
    invalid_citation_ids = [cid for cid in found_citations if cid not in evidence_ids]
    
    # Count valid citations
    valid_citations = [cid for cid in found_citations if cid in evidence_ids]
    citation_count = len(valid_citations)
    
    # Split into sentences (simple split on .!?)
    sentences = re.split(r'[.!?]+', draft_text)
    
    # Factual indicators that require citations
    factual_markers = [
        'according to', 'report', 'study', '%', 'million', 'billion', 
        'survey', 'data', 'researchers', 'found', 'statistics', 
        'research shows', 'analysis reveals', 'figures show'
    ]
    
    # Find sentences with factual markers but no citations
    ...
    for sentence in sentences:
        ...
        # Check if sentence has factual marker
        has_factual_marker = any(marker in sentence.lower() for marker in factual_markers)
        
        if has_factual_marker:
            # Check if sentence has any citation
            has_citation = bool(re.search(citation_pattern, sentence))
            if not has_citation:
                missing_citations_sentences.append(sentence)
    
    passed = (len(missing_citations_sentences) == 0 and len(invalid_citation_ids) == 0)
    
    return {
        'missing_citations_sentences': missing_citations_sentences,
...
def validate_ps_citations(draft_text: str, must_use_min: int) -> Dict[str, Any]:
    """
    Validate that minimum number of provided-source (PS_*) citations are used.
    
    Args:
        draft_text: Generated newsletter text
        must_use_min: Minimum required PS_* citations
    
    Returns:
        Dict with keys:
        - ps_citation_count: int - number of PS_* citations found
        - ps_required: int - minimum required
        - ps_passed: bool - True if requirement met
    """
    ...
    # Count PS_* citations
    ps_citations = [cid for cid in found_citations if cid.startswith('PS_')]
    ps_citation_count = len(set(ps_citations))  # Unique PS_ citations
    
    ps_passed = ps_citation_count >= must_use_min
    
    return {
        'ps_citation_count': ps_citation_count,
        'ps_required': must_use_min,
        'ps_passed': ps_passed
    }
...
Return ONLY valid JSON, no markdown formatting."""
    ...
    def build_prompt(
        ...
        user_prompt = self._build_user_prompt(
            ...
            relevance_brief=relevance_brief,
            retrieval_pack=pack
        )
        ...
        metadata = {
            ...
            'retrieval_pack': asdict(pack) if pack else {},
            'evidence_ids': [ex.id for ex in pack.excerpts] if pack and pack.excerpts else [],
            'topic_signature': {
...
"""
    ...
    def _build_user_prompt(
        ...
        style_metrics: Dict = None,
        relevance_brief: Optional[RelevanceBrief] = None,
        retrieval_pack: Optional[RetrievalPack] = None
    ) -> str:
        """
        ...
        if relevance_brief:
            vps = [it for it in relevance_brief.selected_items if it.source_type in ['voice_pattern', 'editorial_voice']]
            if vps:
        ...
        if relevance_brief:
            ...
            for item in relevance_brief.selected_items[:10]:
                prompt += f"- {item.title} [{item.source_type}] why: {item.why_relevant} use_as: {item.use_as}\n"
            if relevance_brief.exclusions:
                prompt += "Exclusions: " + "; ".join(relevance_brief.exclusions[:5]) + "\n"
            prompt += "\n"
        
        # SECTION 3.7: EVIDENCE PACK (cite with IDs)
        if retrieval_pack and retrieval_pack.excerpts:
            prompt += "# EVIDENCE PACK (cite with IDs like [PS_1], [VP_2], etc.)\n\n"
            prompt += "Use these evidence items in your newsletter. Cite them using their bracket IDs.\n\n"
            for excerpt in retrieval_pack.excerpts[:12]:
                prompt += f"[{excerpt.id}] {excerpt.source}"
                if excerpt.source_type:
                    prompt += f" | {excerpt.source_type}"
                if excerpt.url:
                    prompt += f" | {excerpt.url}"
                prompt += "\n"
                # Truncate text to ~500 chars
                text_snippet = excerpt.text[:500] + "..." if len(excerpt.text) > 500 else excerpt.text
                prompt += f"Snippet: {text_snippet}\n\n"
            prompt += "\n"
        ...
        prompt += "7. 🚨 EVIDENCE PACK CITATION RULES:\n"
        prompt += "   - Every factual claim MUST end with a citation like [PS_1] or [KB_3] from the Evidence Pack\n"
        prompt += "   - If you cannot cite it from the Evidence Pack, write \"needs verification\" or omit the claim\n"
        prompt += "   - Do NOT invent URLs, publications, or studies - only cite IDs from the Evidence Pack\n"
        min_citations = max(3, plan.must_use_provided_sources_min) if plan else 3
        prompt += f"   - Use at least {min_citations} citations in the main story from the Evidence Pack\n"
        prompt += "8. Use only the selected items in the Relevance Brief and Evidence Pack. Do not include large quoted blocks from past newsletters or full knowledge base dumps.\n"
        prompt += "9. Use the ANCHORED OUTLINE bullets and their anchors; do not invent facts outside anchors\n"
        prompt += "10. Incorporate relevant facts with accurate citations using Evidence Pack IDs\n"
        prompt += "11. Make it engaging, practical, and authentic to Paul's voice\n"
...
class NewsletterGenerator:
    ...
    def _build_repair_prompt(
        ...
        failing_draft: str, 
        citation_audit: Dict, 
        ps_audit: Optional[Dict], 
        retrieval_pack_dict: Dict,
        evidence_ids: List[str]
    ) -> str:
        """Build prompt for citation repair loop."""
        
        prompt = "# CITATION REPAIR TASK\n\n"
        prompt += "The draft below has citation issues. Fix ONLY the citations - do not change the content otherwise.\n\n"
        
        # Show evidence pack
        prompt += "# EVIDENCE PACK (allowed citation IDs)\n\n"
        excerpts = retrieval_pack_dict.get('excerpts', [])
        if excerpts:
            for excerpt in excerpts[:12]:
                ...
                prompt += f"[{ex_id}] {source}"
                if source_type:
                    prompt += f" | {source_type}"
                if url:
                    prompt += f" | {url}"
                prompt += "\n"
        ...
        # Report invalid IDs
        if citation_audit['invalid_citation_ids']:
            prompt += f"❌ **Invalid citation IDs used:** {', '.join(citation_audit['invalid_citation_ids'])}\n"
            prompt += "   These IDs are not in the Evidence Pack. Replace with valid IDs or remove.\n\n"
        
        # Report missing citations
        if citation_audit['missing_citations_sentences']:
            prompt += f"❌ **Factual sentences without citations:** {len(citation_audit['missing_citations_sentences'])} found\n"
            prompt += "   Examples:\n"
            for sentence in citation_audit['missing_citations_sentences'][:3]:
                prompt += f"   - \"{sentence[:100]}...\"\n"
            prompt += "   Add citations from Evidence Pack or rewrite to be less factual.\n\n"
        
        # Report PS citation shortfall
        if ps_audit and not ps_audit['ps_passed']:
            prompt += f"❌ **Provided source citations:** {ps_audit['ps_citation_count']}/{ps_audit['ps_required']} required\n"
            prompt += f"   Use more [PS_*] citations from the Evidence Pack.\n\n"
        
        prompt += "\n# FAILING DRAFT\n\n"
        prompt += failing_draft
        prompt += "\n\n"
        
        prompt += "# INSTRUCTIONS\n\n"
        prompt += "1. Fix citations by ONLY using IDs from the Evidence Pack above\n"
        prompt += "2. Add missing citations to factual sentences\n"
        prompt += "3. Replace invalid citation IDs with valid ones or remove the claim\n"
        if ps_audit and not ps_audit['ps_passed']:
            prompt += f"4. Use at least {ps_audit['ps_required']} provided source [PS_*] citations\n"
        prompt += "5. Do NOT add new facts or URLs not in the Evidence Pack\n"
        prompt += "6. Do NOT change the content otherwise - only fix citations\n\n"
        prompt += "Return the complete repaired newsletter:\n"
    ...
    def generate(
        ...
        # ---------------- CITATION VALIDATION + ONE-RETRY REPAIR ----------------
        ...
        citation_audit = validate_citations(content, evidence_ids)
        
        # Run PS citation validation if applicable
        ps_audit = None
        plan_dict = metadata.get('newsletter_plan', {})
        must_use_min = plan_dict.get('must_use_provided_sources_min', 0) if plan_dict else 0
        if must_use_min > 0:
            ps_audit = validate_ps_citations(content, must_use_min)
        
        # Check if repair is needed
        citation_failed = not citation_audit['passed']
        ps_failed = ps_audit is not None and not ps_audit['ps_passed']
        needs_repair = citation_failed or ps_failed
        
        final_content = content
        repaired = False
        
        if needs_repair:
            print(f"⚠️  Citation validation failed - attempting repair...")
            if citation_failed:
                print(f"   - Invalid citation IDs: {citation_audit['invalid_citation_ids']}")
                print(f"   - Missing citations: {len(citation_audit['missing_citations_sentences'])} sentences")
            if ps_failed:
                print(f"   - PS citations: {ps_audit['ps_citation_count']}/{ps_audit['ps_required']}")
            ...
            try:
                ...
                final_content = repair_response.choices[0].message.content
                repaired = True
                
                # Re-validate after repair
                citation_audit = validate_citations(final_content, evidence_ids)
                if must_use_min > 0:
                    ps_audit = validate_ps_citations(final_content, must_use_min)
                
                print(f"✅ Repair complete - Citations now valid: {citation_audit['passed']}")
                if ps_audit:
                    print(f"   PS citations: {ps_audit['ps_citation_count']}/{ps_audit['ps_required']}")
        ...
        # Store citation audit in metadata
        citation_audit_meta = {
            'citations': citation_audit,
            'repaired': repaired
        }
        if ps_audit:
            citation_audit_meta['ps'] = ps_audit
        
        # Extract correct objects for coverage audit
        ...
        if self.prompt_constructor.audit_failed(audit_report):
            ...
            result = build_result(fixed, {'audit': audit_report_fixed, 'citation_audit': citation_audit_meta})
        else:
            result = build_result(final_content, {'audit': audit_report, 'citation_audit': citation_audit_meta})
